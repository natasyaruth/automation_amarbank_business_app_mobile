Feature: SDK Integration for Liveness Detection on iOS
    In order to perform liveness detection in the iOS app
    As a user
    ...
    Scenario: API stores the transaction with Liveness ID
        ...
        When I send a POST request to "active-liveness/abadi/advanceaisdk/ios/store/v1" with the Liveness ID
    ...
    Scenario: User denies camera permissions
        ...
        Then I should receive an event result "Camera is required but access prevented by user settings or administrator policy."
