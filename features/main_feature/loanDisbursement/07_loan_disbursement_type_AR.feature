...
Feature: Loan Disbursement for Loan Type AP - Direct Loan
 ...
 @C98038
 Scenario: Validate request loan disbursement for type Loan AR - Direct Loan when customer have any past due date
  Given I have been on Loan Dashboard to checking if customer Loan AR - Direct AP have any past due date
 ...
 @C98039
 Scenario: Validate request loan disbursement for type Loan AR- Direct Loan when customer do not have any past due date
  Given I have been on Loan Dashboard to checking if customer Loan AR - Direct AR do not have any past due date
  When I click button "Gunakan Limit" in card type Loan AR
  And I do not have any past due date
  Then I direct to page for checking Loan Tenor

 @C98040
 Scenario: Validate request loan disbursement for type Direct Loan AR when Loan Tenor More Than Facility Due Date
  Given I have been on Loan Dashboard to checking if loan tenor more than Facility due date
  When I click button "Gunakan Limit" in card type Direct Loan AR
 ...
 @C9841
 Scenario: Validate Request Loan Disbursement for Type Loan AR- Direct Loan when Loan Tenor Less Than or Equal With Facility Due Date
 ...
 Scenario: Validate Loan Program Information for Type AR - Direct Loan
  ...
  Scenario: Validate page Loan Program Information for type AR and LTV is 100%
    Given I have been on Loan Dashboard to checking if anchor already upload invoice
    When I click button "Gunakan Limit" in card type Loan AR
    And I am on Anchor Detail Page
    And I click "Info Selengkapnya" in limit card for type AR
    Then I should be see Program Loan Information for type AR

@FunctTestLendingSprint6
Scenario: Validate page Loan Program Information for type AR and LTV < 100%
    Given I have been on Loan Dashboard to checking if anchor already upload invoice
    When I click "Info Selengkapnya" in limit card for type AR
    And I am on Anchor Detail Page
    And I click button "Info Selengkapnya" for type AR
    Then I should be see Program Loan Information for type AR below 100
...
Scenario: Validate Users can view only ongoing invoices in the main loan section
 ...
 @C98045
 Scenario: Upload Invoice Picture From Gallery With Type Direct Loan AR
