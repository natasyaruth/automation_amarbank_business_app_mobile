...
When("I input amount higher than active balance", async () => {
  ...
  const amount = activeBalance + 10.0;
  transferPage.inputAmountTransfer(amount);
});

Then(
  "I should see error message {string} in field {string}",
...
Then("I can see BIFAST and SKN", async () => {
  ...
  I.assertEqual(
    actualAdminFeeBifast,
    "Rp " + globalVariable.transfer.adminFeeBIFAST
  );
  I.see("BI Fast");
  ...
  const formattedString = globalVariable.transfer.adminFeeBIFAST.replace(
    /Rp|\./g,
    ""
  );
  ...
  const expectedTotalAmount =
    transferPage.formattedToThreeDigit(numberTotalAmount);
...
Then("I can see BIFAST, SKN and RTGS", async () => {
  ...
  I.assertEqual(
    actualAdminFeeRtgs,
    "Rp " + globalVariable.transfer.adminFeeRTGS
  );
  I.see("RTGS");
  I.see("Dana langsung sampai ke penerima");
  I.see("Nominal transfer:");
...
Then("I can see SKN and RTGS", async () => {
  ...
  const actualTotalAmount = await transferPage.getValueTotalAmount();
  const formattedString = globalVariable.transfer.adminFeeSKN.replace(
    /Rp|\./g,
    ""
  );
  const numberAdminFee = parseInt(formattedString);
...
Then("I can see RTGS", async () => {
  ...
  const actualTotalAmount = await transferPage.getValueTotalAmount();
  const formattedString = globalVariable.transfer.adminFeeRTGS.replace(
    /Rp|\./g,
    ""
  );
  const numberAdminFee = parseInt(formattedString);
...
Then("I can see BIFAST, RTOL and SKN", async () => {
  ...
  I.assertEqual(
    actualAdminFeeRTOL,
    "Rp " + globalVariable.transfer.adminFeeRTOL
  );
  I.see("RTOL");
  I.see("Dana langsung sampai ke penerima");
  I.see("Nominal transfer:");
...
Then("I choose transfer service RTGS", async () => {
  ...
  const numberTotalAmount =
    globalVariable.transfer.amount + globalVariable.transfer.adminFee;
  const expectedTotalAmount =
    transferPage.formattedToThreeDigit(numberTotalAmount);
...
Then("I successfully transferred", async () => {
  ...
  const typeBusiness = (
    await resetStateDao.getAccountType(
  ...
  if (typeBusiness === "1") {
    expectedSenderName = (
      await resetStateDao.getFullName(
        globalVariable.login.userID,
        globalVariable.login.password
      )
    ).ktpName;
    ...
    expectedSenderName = (
      await resetStateDao.getCompanyName(
        globalVariable.login.userID,
        globalVariable.login.password
      )
    ).businessName;
  ...
  const actualSenderAccNumber = (
    await transferPage.getSenderAccNnumber()
  ).replace(/\s+/g, "");
  const expectedSenderAccNumber = (
    await resetStateDao.getAccountNumber(
      globalVariable.login.userID,
      globalVariable.login.password
    )
  ).accountNumber;
  ...
  const actualReceiverAccNumber = (
    await transferPage.getReceiverAccNnumber()
  ).replace(/\s+/g, "");
  I.assertEqual(
    actualReceiverAccNumber,
    globalVariable.friendList.friendListAccNumber
      .replace(/\s+/g, "")
      .replace(/-/g, "")
  );
...
When(
  "I will directly go to page confirmation transfer between Amar Bank",
  async () => {
...
When("I input amount more than active amount", async () => {
  const amountActive = (await transferPage.getAmountActive()).replace(
    /Rp|\./g,
    ""
  );
...
Then(
  "I see message error total amount shouldn't more than active amount",
  async () => {
    ...
    const disabledButton = await I.grabAttributeFrom(
      transferPage.buttons.transfer,
      "enabled"
    );
...
Then(
  "I shouldn't see message error total amount more than active amount",
  async () => {
    I.wait(1);
...
Then("I am on Transfer methode list page", async () => {
  ...
  const actualActiveAmount = (
    await transferPage.getValueAmountActive()
  ).replace(/ /, "");
  ...
  I.assertEqual(
    actualTitleTransferAmount,
    listExpectedTitle.titleAmountTransfer
  );

  const actualTransferAmount = await transferPage.getValueAmountTransfer();
  const expectedTransferAmount = transferPage.formattedToThreeDigit(
    globalVariable.transfer.amount
  );
...
Then("I am on page transfer confirmation", async () => {
  ...
  const actualSenderAccNumber = (
    await transferPage.getConfirmSenderAccNumber()
  ).replace(/\s+/g, "");
  const expectedSenderAccNumber = (
    await resetStateDao.getAccountNumber(
      globalVariable.login.userID,
  ...
  const actualRecipientBankName =
    await transferPage.getConfirmRecipientBankName();
  ...
  const actualRecipientAccNumber = (
    await transferPage.getConfirmRecipientAccNumber()
  ).replace(/[- ]/g, "");
  const expectedRecipientAccNumber =
    globalVariable.friendList.friendListAccNumber.replace(/[- ]/g, "");
  ...
  const expectedNominalTransfer = transferPage.formattedToThreeDigit(
    globalVariable.transfer.amount
  );
  ...
  const expectedAdminFee = transferPage.formattedToThreeDigit(
    globalVariable.transfer.adminFee
  );
  ...
  I.assertEqual(
    actualTotalTransfer,
    "Rp" + globalVariable.transfer.totalAmount
  );
