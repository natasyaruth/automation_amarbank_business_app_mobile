Feature: Apply First Loan With Flagging MSME Using PO Direct PT
   As a customer lead
   ...
   @FunctTestLendingSprint5
   Scenario: User apply first loan PO direct type with flagging MSME PT Perorangan
      Given User on Main Dashboard
      When user click button Ajukan Limit Kredit
      And User select loan type "PO"
      And User on Loan Needs Page
      ...
      And user have completed in stepper Proses KYC & KYB
      And user on Stepper Upload Dokumen
      And user validate description prepare the following documents 'POPT.Perorngan'
      And user click buttton Pilih Metode Upload Dokumen
      And user click button Langsung dari Aplikasi
      And user on Progress Upload Dokumen Page
      And user upload docuemnt "nib"
