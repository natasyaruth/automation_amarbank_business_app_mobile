...
When(
  "I verifying my phone number by entering the code sent to me",
  async () => {
    let actualPhoneNumber = await otpConfirmationPage.getPhoneNumber();
    let expectedPhoneNumber =
      globalVariable.registration.phoneNumber.substring(2);
...
When("I verifying my email by login by user id", async () => {
  ...
  globalVariable.registration.userID = await otpDao.getUserID(
    globalVariable.registration.email
  );

  verificationEmailPage.loginWithUserId(
    globalVariable.registration.userID,
    globalVariable.registration.password,
    globalVariable.registration.email
  );
});
